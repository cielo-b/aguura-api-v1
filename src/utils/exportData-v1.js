const exportData = async (stock, activeDay) => {
  ...
  const adminTitleX =
    pdfDoc.page.width + 15 - pdfDoc.page.margins.right - adminTitleWidth;
  const adminTitleY = 45;
  pdfDoc
    .fontSize(8)
    .font(PoppinsRegular)
    .text(adminTitle, adminTitleX, adminTitleY);
  pdfDoc.x = normalX;
  pdfDoc.moveDown();
  pdfDoc.moveDown();
  ...
  let totalSales = 0;
  let totalProducts = 0;
  let salesData = [["Producer", "Quantity", "Total Price"]];
  for (let company of companies) {
    ...
    const products = sales.flatMap((sale) =>
      sale.products.filter(
        (product) =>
          product.id &&
          product.id.inventoryProduct &&
          product.id.inventoryProduct.company.equals(company.id),
      ),
    );
  ...
  // crates section
  let totalCrates = 0;
  const cratesData = [["Customer", "Rendered Crates"]];
  ...
  const eCrates = await EmptyCrates.find({
    activeDay: activeDay.id,
    stock: stock.id,
  });

...
const newDistributionPoint = catchAsync(async (req, res) => {
  ...
  const distributionPoint = await DistributionPoint.create({
    manager: admin.id,
    location,
    name,
  });
...
const editDistributionPoint = catchAsync(async (req, res) => {
  const distributionPoint = await DistributionPoint.findById(
    req.params.distributionPointId,
  );
  ...
  const distributionPointLocation = location
    ? location
...
const getPointByManager = catchAsync(async (req, res) => {
  let distributionPoint = await DistributionPoint.findOne({
...
const getAllDistributionPoints = catchAsync(async (req, res) => {
  ...
  distributionPoints = await Promise.all(
    distributionPoints.map(async (distributionPoint) => {
...
const getStocks = catchAsync(async (req, res) => {
  ...
  const distributionPoint =
    await DistributionPoint.findById(distributionPointId);

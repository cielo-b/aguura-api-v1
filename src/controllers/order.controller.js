...
const {InventoryProduct, SalesProduct, Sales, Credit, User, Order, Payment, PaymentMethod, DistributionPoint, Producer, Product, ActiveDay, Inventory, Stock, EmptyCrates} = require('../models');
...
const completeOrder = catchAsync(async (req, res) => {
    const {entityType, entityId, id, payments, amountPaid, isFullyPaying} = req.body;
    ...
    if (amountPaid > 0) {
        ...
        } else if (isFullyPaying && !(parseFloat(amountPaid) === parseFloat(order.totalPrice))) {
            return res.status(httpStatus.BAD_REQUEST).json({
                success: false,
...
const inventOrder = catchAsync(async (req, res) => {
    ...
    if (entityType === 'distributionPoint') {
        ...
        for (let product of distributionProducts) {
            const _product = reqProducts.find(p => p.id.toString() === product.product?.toString());
            if (_product) {
        ...
        for (let product of stockProducts) {
            const distProduct = distributorProducts.find(p => p.productName === product.productName);
            const _product = reqProducts.find(p => p.id.toString() === distProduct?._id.toString());
            if (_product) {
                const inventoryProduct = {
                    name: product.name,
                    quantity: parseFloat(_product.quantity),
        ...
        const inventory = await Inventory.create({activeDay, products: iProducts, totalPrice: iTotalPrice, stock: stock._id, description: iDescription});
        if (inventory) {
            if (sales) {
                sales.inventory = inventory.id;
                await sales.save({validateBeforeSave: false});
            ...
            for (let i = 0; i < reqProducts.length; i++) {
                let reqProduct = reqProducts[i];
                const distProduct = distributorProducts.find(p => p._id.toString() === reqProduct.id.toString());
                const _product = stockProducts.find(p => p.productName.toString() === distProduct.productName.toString());
                let product = null;
                if (_product) {
                ...
                if (producer) {
                    if (product?.product) {
                        ...
                        if (p.producer.toString() === producer.id.toString()) {
                            total += parseFloat(distProduct.price * parseFloat(reqProduct.quantity));
                        }
                    }
                }
            }
            ...
            if (producer) {
                ...
                let stockIndex = stocks.findIndex(s => s.id.toString() === stock._id.toString());
            ...
            if (distributor) {
                ...
                if (stockIndex !== -1) {
                    ...
                    stocks.push({id: stock._id, totalPurchases: parseFloat(order.totalPrice)});
                }
...
const newDistributorOrder = catchAsync(async (req, res) => {
    ...
    const {distributorId, producerId, products: reqProducts, activeDay} = req.body;
    const distributionPoint = await DistributionPoint.findById(distributorId).populate('manager');
    ...
    const order = await Order.create({totalPrice, products, description, distributionPoint: distributionPoint.id, producer: producer.id, isMine: true, customerName: distributionPoint.manager.name, phone: distributionPoint.manager.phone, activeDay});
...
const getProducerOrders = catchAsync(async (req, res) => {
    ...
    let orders = await Order.find({producer: producer.id, isCompleted: isCompleted});
...
const getDistributorOrders = catchAsync(async (req, res) => {
    ...
    let orders = [];
    if (isMine) {
        let queryObj = {distributionPoint: distributionPoint.id, isCompleted, status, producer: {$ne: null}};
        orders = await Order.find(queryObj);
        orders = await Promise.all(orders.map(async order => {
            const producer = await Producer.findById(order.producer).populate('manager');
            return {
                id: order.id,
                description: order.description,
                name: producer.name,
        ...
        let queryObj = {distributionPoint: distributionPoint.id, isCompleted, producer: null};
        orders = await Order.find(queryObj);
        orders = await Promise.all(orders.map(async order => {
...
const newStockOrder = catchAsync(async (req, res) => {
    ...
    for (let i = 0; i < reqProducts.length; i++) {
        ...
        let saleProduct = await SalesProduct.findOne({inventoryProduct: product.id});
    ...
    const order = await Order.create({totalPrice, products, description, distributionPoint: distributionPoint.id, stock: stock.id, isMine: true, customerName: stock.admin.fullName, phone: stock.admin.phone});
...
const adminOrders = catchAsync(async (req, res) => {
    ...
    let orders = await Order.find({isCompleted: req.query.isCompleted, stock: req.query.stockId, distributionPoint: null}, {products: 0}).populate('customer');
    orders = orders.map(o => {
        return {

...
const generatePDF = async (stock, activeDay) => {
  ...
  doc
    .fontSize(18)
    .text(`${config.name}  ${activeDay.name} Daily Report. \n\n\n\n`, {
      underline: true,
    });
  ...
  if (inventories.length > 0) {
    ...
    for (const inventory of inventories) {
      ...
      for (let product of products) {
        ...
        doc
          .fontSize(10)
          .text(`Unit Price : ${formatNumber(product.unitPrice)} Rwf\n`);
        doc
          .fontSize(10)
          .text(`Total Price : ${formatNumber(product.totalPrice)} Rwf\n\n`);
        total += product.totalPrice;
      }
      doc
        .fontSize(12)
        .text(`Total : ${formatNumber(total)} Rwf \n\n\n`, { underline: true });
      totalInventoryPrice += total;
    }
    doc
      .fontSize(13)
      .text(
        `Total Inventory: ${formatNumber(totalInventoryPrice)} Rwf \n\n\n\n`,
        { underline: true },
      );
  } else {
    doc
      .fontSize(13)
      .text(`No Inventories Were Made On ${activeDay.name}.\n\n\n\n`);
  }
  ...
  if (sales.length > 0) {
    ...
    for (const sale of sales) {
      ...
      for (const product of sale.products) {
        ...
        doc
          .fontSize(10)
          .text(`Quantity : ${formatNumber(product.quantity)} \n`);
        doc
          .fontSize(10)
          .text(`Unit Price : ${formatNumber(product.unitPrice)} Rwf\n`);
        doc
      ...
      if (!sale.isFullyPaid) {
        doc
          .fontSize(10)
          .text(
            `Remaining : ${formatNumber(sale.totalPrice - sale.amountPaid)} Rwf \n`,
          );
        remaining += sale.totalPrice - sale.amountPaid;
      }
      doc
        .fontSize(12)
        .text(`Total : ${formatNumber(sale.totalPrice)} Rwf \n\n\n`, {
          underline: true,
        });
    ...
    doc
      .fontSize(13)
      .text(`Total Paid: ${formatNumber(totalSalesPrice - remaining)} Rwf \n`);
    doc.fontSize(13).text(`Total Reaming: ${formatNumber(remaining)} Rwf \n`);
    doc
      .fontSize(13)
      .text(`Total Sales: ${formatNumber(totalSalesPrice)} Rwf \n\n\n\n`, {
        underline: true,
      });
  } else {
  ...
  if (creditSales.length > 0) {
    ...
    for (const sale of creditSales) {
      ...
      doc
        .fontSize(10)
        .text(`Amount To Be Paid : ${formatNumber(sale.totalPrice)} Rwf\n`);
      doc
        .fontSize(10)
        .text(`Amount Paid : ${formatNumber(sale.amountPaid)} Rwf\n`);
      doc
        .fontSize(10)
        .text(
          `Amount Remaining : ${formatNumber(sale.totalPrice - sale.amountPaid)} Rwf\n\n\n`,
        );
    ...
    doc
      .fontSize(13)
      .text(`Total Credit: ${formatNumber(totalCredit)} Rwf \n\n\n\n`, {
        underline: true,
      });
  } else {
  ...
  if (crates.length > 0) {
    ...
    for (let crate of crates) {
      ...
      doc
        .fontSize(10)
        .text(
          `${crate.customerName} ${crate.customerPhone ? crate.customerPhone : ""}\n\n`,
        );
      ...
      for (let product of products) {
        doc
          .fontSize(10)
          .text(
            `${product.name} => given: ${formatNumber(product.given)} | returned: ${formatNumber(product.returned)} | remaining: ${formatNumber(product.remaining)} \n `,
            { underline: true },
          );
      ...
      doc
        .fontSize(10)
        .text(
          `Total => given:${formatNumber(given)} | returned:${formatNumber(returned)} | remaining:${formatNumber(remaining)}\n\n`,
        );
      givenC += given;
      returnedC += returned;
    ...
    doc
      .fontSize(12)
      .text(
        `Total => given:${formatNumber(parseInt(givenC))} | returned:${formatNumber(parseInt(returnedC))} | remaining:${formatNumber(parseInt(remainingC))}\n\n\n\n`,
      );
  } else {
    doc
      .fontSize(13)
      .text(`No Crates Were Rendered On ${activeDay.name}.\n\n\n\n`);
  }
  ...
  if (emptyCrates.length > 0) {
    ...
    doc
      .fontSize(13)
      .text(`No Empty Crates Recorded On ${activeDay.name}.\n\n\n\n`);
  }
  ...
  for (const m of methods) {
    ...
    for (let p of payments) {
      doc
        .fontSize(10)
        .text(
          `${p.customerName} ${p.customerPhone} : ${formatNumber(p.amount)} Rwf\n`,
        );
  ...
  doc
    .fontSize(10)
    .text(
      "Product Name | Initial Inventory | Added Today | Final Inventory \n\n",
    );
  ...
  for (let product of iProducts) {
    doc
      .fontSize(10)
      .text(
        `${product.name} | initial ${formatNumber(product.prevDayRemaining)} | added ${formatNumber(product.dailyAdded)} | remaining ${formatNumber(product.totalAvailable)}\n\n`,
      );
  }
...
async function getEntityById(entityType, entityId) {
  ...
  if (entityType === "distributionPoint")
    return await DistributionPoint.findById(entityId);
...
const downloadReport = catchAsync(async (req, res) => {
  ...
  const pdfFileName =
    activeDay.type === "day"
      ? await exportData(entity, activeDay)
      : await monthlReport(entity.name, activeDay);
  const url = config.url + "/public/reports/" + pdfFileName;

...
const editInventory = catchAsync(async (req, res) => {
  ...
  let entityType = inventory.producer
    ? "producer"
    : inventory.distributionPoint
      ? "distributionPoint"
      : "stock";
  let entity = inventory.producer
    ? await Producer.findById(inventory.producer)
    : inventory.distributionPoint
      ? await DistributionPoint.findById(inventory.distributionPoint)
      : await Stock.findById(inventory.stock);

  let manager = await User.findById(
    entityType === "stock" ? entity.admin : entity.manager,
  );
  ...
  for (let i = 0; i < initials.length; i++) {
    ...
    if (iP) {
      let p =
        entityType === "producer"
          ? await Product.findById(iP.id)
          : await InventoryProduct.findById(iP.id);
    ...
    if (manager.country === "rwanda" && !ebmUpdated) {
      ...
      if (response.resultCd !== "000") {
        return res.status(httpStatus.CREATED).json({
          success: false,
          message:
            "Inventory Recorded Into Aguura But Failed Into EBM, Plz Delete This Inventory And Try Again.",
        });
      } else {
        // update stock Items master
        for (let i = 0; i < products.length; i++) {

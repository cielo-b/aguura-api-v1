...
const {User, Producer, Product, DistributionPoint, ActiveDay} = require('../models');
...
const newProducer = catchAsync(async (req, res) => {
    ...
    const admin = await userService.createUser({fullName, phone, password, role: 'producer', monthlyPayment: parseFloat('100')});
...
const getProducerByManager = catchAsync(async (req, res) => {
    ...
    const activeDay = await ActiveDay.findOne({producer: producer.id, isActive: true});
...
const getAllProducers = catchAsync(async (req, res) => {
    ...
    producers = await Promise.all(producers.map(async (producer) => {
...
const getDistributors = catchAsync(async (req, res) => {
    ...
    distributors = await Promise.all(distributors.map(async (d) => {
        const distributor = await DistributionPoint.findById(d.id).populate('manager');
        if (distributor) {
            return {
...
const getCustomers = catchAsync(async (req, res) => {
    ...
    let customers = await Promise.all(producer.customers.map(async c => {

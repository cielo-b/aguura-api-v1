...
async function updateProducerInventory(initials, distributor) {
  for (const iProduct of initials) {
    const inP = await InventoryProduct.findOne({
...
async function processProducts(reqProducts, entityType) {
  ...
  for (const reqProduct of reqProducts) {
    const product =
      entityType === "producer"
        ? await Product.findById(reqProduct.id)
...
async function updateInventoryProducts(
  distributorId,
  reqProducts,
  inventoryId,
  producer,
  ...
  const distributionProducts = await InventoryProduct.find({
    distributionPoint: distributorId,
  });
  const iProducts = [];
  let iTotalPrice = 0;
  let iDescription = "";
  ...
  for (const product of distributionProducts) {
    const reqProduct = reqProducts.find(
  ...
  let distributorIndex = distributors.findIndex(
    (d) => d.id.toString() === distributorId.toString(),
  );
...
async function updateStockProducts(
  stockId,
  reqProducts,
  inventoryId,
  iPurchase,
    ...
    if (producer) {
      if (product.product) {
        ...
        if (p) {
          ...
          if (saleProduct) {
            total +=
              parseFloat(saleProduct.price) * parseFloat(reqProduct.quantity);
          }
        }
      }
    }
  ...
  if (producer) {
    ...
    let stockIndex = stocks.findIndex(
      (d) => d.id.toString() === stockId.toString(),
    );
...
async function updateInventoryProductsForNonProducers(
  reqProducts,
  iPurchase,
  customerId,
) {
  ...
  if (producer) {
    ...
    let stockIndex = customers.findIndex(
      (d) => d.id.toString() === customerId.toString(),
    );
...
async function handleCredit(
  ...
  if (credit) {
    ...
    credit = await Credit.create({
      ...
      [entityType]: entityId,
      sales: sale.id,
      totalAmount: totalPrice - amountPaid,
      description,
      customerName,
      customerPhone,
    });
    if (!credit) {
      return res.status(httpStatus.OK).json({
        success: true,
        message:
          "Sales recorded successfully, but credit failed to be recorded.",
      });
    }
  }
}
...
const generateEBMRequestData = (products, manager, entity) => {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    totalTxAmt =
      parseFloat(totalTxAmt) +
      parseFloat(((18 / 118) * product.totalPrice).toFixed(2));
...
const newSales = catchAsync(async (req, res) => {
  ...
  let {
    ...
    stock,
    customerTin,
  } = await getCustomerDetails(customerId, req.body);
  ...
  if (!isFullyPaid) {
    ...
    if (!credit) {
      return res.status(httpStatus.CREATED).json({
        success: true,
        message:
          "Sales Eecorded Successfully But, Credit Failed To Te Recorder.",
      });
    }
  }
  ...
  if (entityType === "producer") {
    ...
    if (inventory) {
      ...
      let stockIndex = stocks.findIndex(
        (d) => d.id.toString() === stock._id.toString(),
      );
    ...
    for (let i = 0; i < reqProducts.length; i++) {
      ...
      let product = await InventoryProduct.findById(
      ...
      if (producer) {
        total +=
          parseFloat(salesProduct.price) * parseFloat(reqProduct.quantity);
      }
    ...
    if (producer) {
      let customers = producer.customers;
      let stockIndex = customers.findIndex(
        (d) => d.id.toString() === customer.toString(),
      );
...
const editSales = catchAsync(async (req, res) => {
  ...
  if (!isFullyPaid && amountPaid > 0) {
    ...
    if (parseFloat(totalPayments) !== parseFloat(amountPaid)) {
      return res.status(httpStatus.BAD_REQUEST).json({
        success: false,
        message:
          "Total payment methods amount must equal the total amount paid.",
      });
    }
  }
  ...
  if (entityType === "producer") {
    ...
    await updateInventoryProductsForNonProducers(
      reqProducts,
      purchase,
      sale.customer,
    );
